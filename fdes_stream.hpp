...
namespace fr {
  ...
  namespace socket {
    ...
    public:
      ...
      fdes_stream(int fdes) : fdes(fdes), buf_in(std::make_shared<__gnu_cxx::stdio_filebuf<char > >(fdes, std::ios_base::in)), buf_out(std::make_shared<__gnu_cxx::stdio_filebuf<char > >(fdes, std::ios_base::out)), stream_in(buf_in.get()), stream_out(buf_out.get())
      {
      }
